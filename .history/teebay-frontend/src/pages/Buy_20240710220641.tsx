// Buy or Rent Modal page
import React from "react";
import { Button, Modal, Datepicker, Kbd } from "flowbite-react";
import ConfirmBuy from "./ConfirmBuy";
import { useMutation, useQuery } from "@apollo/client";
import { CREATE_RENT_ITEM, PRODUCT } from "../queries/product.ts";
...
const Buy = () => {
  ...
  return (
    <div className="mt-10 max-w-md mx-auto bg-white md:max-w-2xl border-spacing-2">
      <div className="md:flex border border-black">
        <div className="w-full p-4">
          <h2 className="text-gray-900 font-bold text-2xl mb-2">
            {data?.product?.title}
          </h2>
          <p className="text-gray-700 font-semibold text-sm mb-1 mt-5">
            {data?.product?.categories?.map((cat: Category) => (
              <Kbd key={cat.id} className="mr-0.5">
                {cat?.name}
              </Kbd>
            ))}
          </p>
          <p className="text-gray-700 text-lg mb-2 mt-2">
            Price: ${data?.product?.price}
          </p>
          <p className="text-gray-700 text-lg mb-2">
            Rent per hour: ${data?.product?.rent}
          </p>
          <p className="text-gray-600 text-base mb-4">Product Description : {data?.product?.description}
          </p>
